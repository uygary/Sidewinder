...
namespace Sidewinder.Core.Updater
{
    ...
    public class EmbeddedUpdateAgent : IUpdateAgent
    {
        ...
        public bool Execute()
        {
            Console.WriteLine("\tRunning Application Update Pipeline...");
            ...
            myConfig.TargetPackages.ToList().ForEach(tp =>
                                                         {
                                                             Console.Write("\t\t\t{0}", (object) tp.Value.Name);

                                                             if (tp.Value.Version != null)
                                                                 Console.Write(" v{0}", (object) tp.Value.Version);

                                                             if (string.Compare(Constants.NuGet.OfficialFeedUrl,
                                                                                tp.Value.NuGetFeedUrl,
                                                                                StringComparison.InvariantCultureIgnoreCase) != 0)
                                                                 Console.Write(" ({0})", (object) tp.Value.NuGetFeedUrl);
